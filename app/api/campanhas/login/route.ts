...
import { CLEAR_COOKIE, authConfigured, resolveLogin, sessionCookie } from "@/lib/campanhas/auth";
...
export async function POST(request: NextRequest) {
  ...
  if (!authConfigured()) {
    return NextResponse.json(
      { error: "Acesso ainda não configurado. Defina CAMPANHAS_ADMIN_PASSWORD." },
      { status: 503 }
    );
  }
  ...
  if (!session) return NextResponse.json({ error: "Senha incorreta." }, { status: 401 });
