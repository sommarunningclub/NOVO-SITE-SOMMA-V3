...
import {
  ...
  senhaCorreta,
  tokenDeAcesso,
} from "@/app/ppt-estacao-somma-2/auth";
...
export async function POST(req: Request) {
  ...
  const limite = await rateLimit(`ppt:ppt-estacao-somma-2:${ip}`, 12, 600);
  if (!limite.ok) {
  ...
  if (!configurado()) {
    console.error("[ppt-estacao-somma-2] Código de acesso não configurado.");
