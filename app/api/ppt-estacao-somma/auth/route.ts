...
import {
  ...
  senhaCorreta,
  tokenDeAcesso,
} from "@/app/ppt-estacao-somma/auth";
...
export async function POST(req: Request) {
  ...
  const limite = await rateLimit(`ppt:ppt-estacao-somma:${ip}`, 12, 600);
  if (!limite.ok) {
  ...
  if (!configurado()) {
    console.error("[ppt-estacao-somma] Código de acesso não configurado.");
