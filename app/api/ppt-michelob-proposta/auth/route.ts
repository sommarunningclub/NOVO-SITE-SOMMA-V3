...
import {
  ...
  senhaCorreta,
  tokenDeAcesso,
} from "@/app/ppt-michelob-proposta/auth";
...
export async function POST(req: Request) {
  ...
  const limite = await rateLimit(`ppt:ppt-michelob-proposta:${ip}`, 12, 600);
  if (!limite.ok) {
  ...
  if (!configurado()) {
    console.error("[ppt-michelob-proposta] Código de acesso não configurado.");
