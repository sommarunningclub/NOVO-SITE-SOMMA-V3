 ...
 * `drive` para movimento contínuo de esteira, `none` para scrub.
 */
export const EASE = {
  out: "expo.out",
  soft: "power3.out",
...
/** Aparelho fraco ou tela pequena: cortamos parallax, blur e sticky pesado. */
 ...
 *
 * Todo motion vive dentro de um `gsap.context()` amarrado ao elemento, então o
 * cleanup mata timelines e ScrollTriggers da seção sem vazar entre navegações.
 * Com `prefers-reduced-motion` nada é registrado — o CSS já deixa o conteúdo
 * visível por padrão.
 */
...
/**
 * Sobe o conteúdo de dentro das janelas `.dst-mask`.
 *
 * `fromTo` (e não `to`) de propósito: o estado escondido é definido pelo CSS
 * via `data-motion`, e um `to` partindo do estado computado corre o risco de
 * animar de 0 para 0 se o atributo chegar depois. Aqui o GSAP é dono dos dois
 * extremos, então a revelação acontece independentemente dessa ordem.
 */
...
/** Entrada lateral — usada nos passos e nos cards de unidade. */
...
/** Contador com scrub: o número acompanha a leitura. */
