...
/** Tela de acesso: 6 dígitos, verificação no servidor e cookie assinado. */
export function Gate() {
  ...
  const enviar = useCallback(
    async (valor: string) => {
      ...
      setErro(false);
      try {
        const r = await fetch("/api/ppt-estacao-somma-2/auth", {
          method: "POST",
  ...
  return (
    <main className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-[#0A0A0A] px-5 py-16 text-white">
      ...
      />
      <div className="absolute inset-0 bg-gradient-to-b from-[#0A0A0A]/85 via-[#0A0A0A]/88 to-[#0A0A0A]" aria-hidden />

      <div className="relative z-10 w-full max-w-md text-center">
        <Lockup className="justify-center" size="md" />

        <div className="mx-auto mt-8 h-px w-16 bg-white/30" aria-hidden />

        <p className="mt-7 font-display text-[11px] font-semibold uppercase tracking-[0.4em]" style={{ color: ORANGE }}>
          Apresentação executiva
        </p>
        <h1 className="mt-4 font-display text-4xl font-bold uppercase leading-[0.9] tracking-tight sm:text-5xl">
          Estação SOMMA
        </h1>
        <p
          className="mt-3 text-2xl italic text-white/80"
          style={{ fontFamily: "var(--font-editorial), Georgia, serif" }}
        >
          Café, Cultura e Movimento
        </p>
        ...
        >
            ...
            <div className="pointer-events-none flex justify-center gap-2 sm:gap-3">
              {cells.map((d, i) => {
                ...
                return (
                  <span
                    key={i}
                    className="flex h-14 w-11 items-center justify-center border font-display text-2xl font-bold tabular-nums transition-colors sm:h-16 sm:w-12 sm:text-3xl"
                    style={{
                      borderColor: erro
                        ? ORANGE
                        : d
          ...
          <button
            ...
            className="mt-2 w-full py-4 font-display text-sm font-semibold uppercase tracking-[0.15em] text-white transition-opacity disabled:cursor-not-allowed disabled:opacity-35"
        ...
        <p className="mt-10 text-[11px] text-white/25">SOMMA Club · Brasília, DF · 2026</p>
      </div>
    </main>
  );
}
