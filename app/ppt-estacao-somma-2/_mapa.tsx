...
/**
 * Sobrevoo do novo espaço, no Parque da Cidade.
 *
 * Usa os tiles fotorrealistas do Google (biblioteca `maps3d`): abre com o
 * Parque inteiro, desce até o galpão na beira do lago e fica orbitando baixo,
 * como um drone do SOMMA monitorando a área. Por cima, um HUD discreto com REC,
 * tempo de voo, coordenadas, altitude e rumo lidos da câmera em tempo real. Se
 * o 3D não carregar (chave sem permissão, sem WebGL, rede ruim), cai para a
 * imagem de satélite estática no mesmo ponto, com um sobrevoo simulado por
 * movimento lento da imagem e telemetria animada.
 */
...
/**
 * O espaço hoje registrado como #VEMPROPLAY, na SRPS Estacionamento 10 do
 * Parque da Cidade (Google Places, ago. 2026). O nome do projeto ainda está em
 * aberto, então aqui fica só a referência de onde o lugar é.
 */
export const NOVO_ESPACO = {
  nome: "Estação SOMMA",
  referencia: "#VEMPROPLAY",
  lat: -15.8024,
  lng: -47.9075,
  endereco: "SRPS Estacionamento 10 · Parque da Cidade Sarah Kubitschek · Brasília, DF",
  plusCode: "53XV+24 Brasília, Plano Piloto",
  maps: "https://maps.app.goo.gl/?q=-15.8023865,-47.9071776",
} as const;

/** Enquadramento inicial: o Parque inteiro, visto do sul. */
const CAMERA_PARQUE = { center: { lat: -15.8035, lng: -47.9085 }, range: 6200, tilt: 52, heading: -18 };
/** Altitude do terreno na região do espaço, em metros. */
const ALTITUDE_TERRENO = 1100;
/** Órbita do drone: baixa e inclinada, para ler o volume do galpão e o entorno. */
...
export function MapaNovoEspaco() {
  ...
  useEffect(() => {
    ...
    const tick = (agora: number) => {
      ...
      if (estado === "pronto" && mapa?.center) {
        ...
        setTele({
          lat: NOVO_ESPACO.lat + Math.cos(a) * r,
          lng: NOVO_ESPACO.lng + Math.sin(a) * r,
    ...
    async function montar() {
      try {
        ...
        const marcador = new Marker3DElement({
          position: { lat: NOVO_ESPACO.lat, lng: NOVO_ESPACO.lng, altitude: 60 },
          altitudeMode: AltitudeMode.RELATIVE_TO_GROUND,
          extruded: true,
          label: NOVO_ESPACO.nome,
        });
        ...
        const camera = {
          center: { lat: NOVO_ESPACO.lat, lng: NOVO_ESPACO.lng, altitude: ALTITUDE_TERRENO },
          tilt: TILT_DRONE,
          range: RANGE_DRONE,
          heading: -30,
        };
  ...
  const estatico = CHAVE
    ? `https://maps.googleapis.com/maps/api/staticmap?center=${NOVO_ESPACO.lat},${NOVO_ESPACO.lng}&zoom=18&size=640x640&scale=2&maptype=satellite&key=${CHAVE}`
    : null;
    ...
    <div className="absolute inset-0 overflow-hidden bg-[#0A0A0A]">
      ...
      {estado === "fallback" && estatico ? (
        ...
        <img
          ref={foto}
          src={estatico}
          alt="Vista de satélite do novo espaço no Parque da Cidade"
