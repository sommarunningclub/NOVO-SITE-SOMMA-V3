...
/**
 * Tela de acesso da apresentação de formatos simplificados.
 *
 * Mesma mecânica das outras rotas do deck, porém envia para o endpoint próprio
 * desta proposta, que grava um cookie separado.
 */
export function Gate() {
  ...
  const enviar = useCallback(
    async (valor: string) => {
      ...
      setErro(false);
      try {
        const r = await fetch("/api/ppt-michelob-nova-proposta/auth", {
          method: "POST",
  ...
  return (
    <main className="relative flex min-h-screen flex-col items-center justify-center overflow-hidden bg-[#060B1C] px-5 py-16 text-white">
      ...
      />
      <div
        className="absolute inset-0 bg-gradient-to-b from-[#060B1C]/80 via-[#060B1C]/85 to-[#060B1C]"
        aria-hidden
      />

      <div className="relative z-10 w-full max-w-md text-center">
        <Lockup className="justify-center" size="sm" />
        ...
        <h1 className="mt-4 font-display text-4xl font-bold uppercase leading-[0.9] tracking-tight sm:text-5xl md:text-6xl">
          Sábados do
          <br />
          <span style={{ color: RED }}>Somma Club</span>
        </h1>
