...
import { POCKET_REGUA } from "./_dados";

/**
 * Simulação do Ultra Pass Pocket.
 *
 ...
 *
 * A régua é a das duas datas da proposta (29/08 e 26/09), então o componente
 * acompanha o tamanho de POCKET_REGUA em vez de assumir quatro selos.
 */
...
const TOTAL_SELOS = POCKET_REGUA.length;
...
const LADO_DA_MARCA = [
  {
    ...
    texto:
      "Nome, telefone e e-mail entram no CRM do Somma com a origem da campanha. É o começo da leitura de quem a marca alcançou.",
  },
  {
    titulo: "Presença medida no dia 29",
    texto:
      "O check-in do primeiro sábado vira número: quantos apareceram, quantos são da base Somma e quantos vieram de fora.",
  },
  {
    titulo: "Retorno medido no dia 26",
    texto:
      "O segundo selo mostra quem voltou depois de um mês. É a leitura de retenção que uma ativação de um dia não entrega, e é ela que sustenta a decisão de escalar.",
  },
];
...
export function UltraPassSim() {
  ...
  const proximo = POCKET_REGUA[Math.min(selos, TOTAL_SELOS - 1)];
  const conquistado = selos > 0 ? POCKET_REGUA[selos - 1] : null;
  // Índice 0 é o cadastro; 1 e 2 são os check-ins das duas datas. Antes de
  // emitir o pass, a leitura é sempre a do cadastro.
        ...
        <header className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-3 max-w-xl text-[14px] leading-relaxed text-white/55">
              ...
              proposta para as duas datas, 29 de agosto e 26 de setembro.
            </p>
          </div>
          <Link
        ...
        <div className="mt-10 grid items-start gap-8 lg:grid-cols-[minmax(0,360px)_minmax(0,1fr)]">
                ...
                {!emitido ? (
                  ...
                  <div className="mt-6">
                    ...
                    <p className="mt-3 text-[13px] leading-relaxed text-white/55">
                      Dois sábados, dois selos. Complete o pass e retire seu prêmio no dia 26 de setembro.
                    </p>
                  ...
                  <div className="mt-6">
                    ...
                    <div className="mt-4 flex items-center gap-3.5 rounded-2xl border border-white/10 bg-white/[0.04] p-3.5">
                      ...
                      <div>
                        <p className="text-[12px] font-medium text-white/85">
                          {completo ? "Pass completo" : `Check-in de ${POCKET_REGUA[selos].curto}`}
                        </p>
                        <p className="mt-0.5 text-[10px] text-white/40">
                          {completo ? "Apresente para retirar o prêmio" : "Apresente na chegada"}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-2">
                      {POCKET_REGUA.map((d) => {
                        ...
                        return (
                          ...
                          >
                            {feito ? <Check /> : d.selos}
                            <span className="mt-0.5 text-[8px] font-normal opacity-60">{d.curto}</span>
                          </span>
                        );
                      })}
                    </div>
                    ...
                    >
                      ...
                      <p className="mt-1.5 text-[12.5px] leading-relaxed text-white/75">
                        {completo
                          ? POCKET_REGUA[TOTAL_SELOS - 1].beneficio
                          : (conquistado ?? proximo).beneficio}
                      </p>
                      {!completo && (
                        <p className="mt-2 text-[11px] text-white/40">
                          Falta o selo de {POCKET_REGUA[TOTAL_SELOS - 1].rotulo} para o prêmio da
                          temporada.
                        </p>
                      )}
                    </div>
                    ...
                    {completo ? (
                      <div className="mt-5 rounded-2xl border border-white/10 bg-white/[0.03] p-3.5 text-center">
                        ...
                        <p className="mt-1 text-[11px] leading-relaxed text-white/50">
                          Somma Day. Apresente o pass completo para retirar o prêmio.
                        </p>
                      </div>
                    ) : (
                      <button
                        ...
                        style={{ backgroundColor: RED }}
                      >
                        Simular check-in · {POCKET_REGUA[selos].curto}
                      </button>
                    )}
                  </div>
                )}
          ...
          <div className="space-y-4">
            ...
            <p className="text-[11px] leading-relaxed text-white/30">
              ...
              campanha, o pass roda sobre a base do Somma, com check-in por QR Code na chegada das duas
              datas.
            </p>
          </div>
        </div>
