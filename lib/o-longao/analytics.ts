...
/**
 * Camada central de analytics do O LONGÃO.
 *
 * Todo evento de tracking da LP passa por aqui — nada de `gtag`/`dataLayer`
 * solto no meio dos componentes. O GTM e o GA4 já vivem no layout raiz do
 * site; esta camada empurra para o `dataLayer` (GTM), chama `gtag` quando
 * existe e mapeia para o Meta Pixel apenas se a página já tiver `fbq`.
 */

const EVENT_SLUG = "o-longao-2026";
...
const STORAGE_KEY = "lgo_attrib";

/**
 * Captura UTMs/referral na primeira visita e persiste em sessionStorage, para
 * que a inscrição (que acontece em outra rota) saiba de onde a crew veio.
 */
