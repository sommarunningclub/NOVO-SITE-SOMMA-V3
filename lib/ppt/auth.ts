 ...
 *
 * Duas coisas mudaram em relação à versão por deck que isto substitui:
 *
 * 1. **Sem código padrão.** Cada deck tinha `"258510"` ou `"101010"` escrito no
 *    fonte — versionado no Git e repetido no README. Agora, sem a variável de
 *    ambiente, o deck simplesmente não abre.
 * 2. **Segredo de assinatura separado do código.** O cookie era assinado com
 *    uma chave derivada do próprio código de acesso, então quem descobria o
 *    código podia forjar o cookie de qualquer um. Agora quem assina é o
 *    `AUTH_SECRET` da aplicação, com um propósito por deck: um cookie do
 *    michelob não vale no silver-care, e nenhum dos dois deriva do código.
 */
